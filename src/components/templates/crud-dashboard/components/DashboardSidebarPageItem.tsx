// src/components/templates/crud-dashboard/components/DashboardSidebarPageItem.tsx
'use client';
    ...
    <React.Fragment>
      <ListItem
        ...
        {...(nestedNavigation && mini
          ...
          : {})}
        sx={{ display: 'block', py: 0, px: 1, overflowX: 'hidden' }}
      >
        <ListItemButton
          selected={selected}
          disabled={disabled}
          sx={{ height: mini ? 50 : 'auto' }}
          {...(nestedNavigation && !mini ? { onClick: handleClick } : {})}
          {...(!nestedNavigation
            ? {
                LinkComponent,
                ...(hasExternalHref
        ...
        >
          ...
          {!mini ? (
            <ListItemText primary={title} sx={{ whiteSpace: 'nowrap', zIndex: 1 }} />
          ) : null}
        ...
        {nestedNavigation && mini ? (
          <Grow in={isHovered}>
            <Box sx={{ position: 'fixed', left: MINI_DRAWER_WIDTH - 2, pl: '6px' }}>
              <Paper elevation={8} sx={{ pt: 0.2, pb: 0.2, transform: 'translateY(-50px)' }}>
